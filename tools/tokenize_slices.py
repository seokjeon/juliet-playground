...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Tokenize generated slice files and plot token count distribution.'
    )
    parser.add_argument(
        '--slice-dir',
        type=Path,
...
def list_slice_files(slice_dir: Path) -> list[Path]:
    return sorted(
        p for p in slice_dir.iterdir() if p.is_file() and p.suffix.lower() in ALLOWED_SUFFIXES
    )


def process_slices(
    slice_dir: Path, tokenizer: RobertaTokenizer
