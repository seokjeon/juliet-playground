...
def generate_signatures(
    ...
    for testcase_dir_name in sorted(
        name for name in os.listdir(input_dir) if os.path.isdir(os.path.join(input_dir, name))
    ):
        if testcase_dir_name == 'analysis':
            continue
        print(f'Generating signatures from {testcase_dir_name}')
        report = input_dir / testcase_dir_name / 'infer-out' / 'report.json'
        ...
        group_key = get_group_key(testcase_dir_name)
        ...
        taint_alarms = [alarm for alarm in alarms if alarm.get('bug_type') == 'TAINT_ERROR']
