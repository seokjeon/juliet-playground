...
`tools/stage/pipeline.py` 기준으로 기록합니다.
...
- 각 단계는 다음 네 가지를 중심으로 적습니다.
  ...
  - 무엇을 입력으로 받는가
  - 무엇을 다음 단계에 넘기는가
  - `pipeline_run.py`가 어떤 산출물 존재를 실제로 검사하는가
- 테스트를 쓸 때는 가능하면
...
6. taint config 선택 (`pipeline_run.py` 내부 분기)
...
`tools/run-epic001-pipeline.py` / `tools/stage/pipeline.py` 는 아래 입력을 받습니다.
...
- 이 단계 뒤에 `pipeline_run.py` 가 generated taint config 존재 여부를 보고
  이후 Infer에 쓸 config를 결정합니다.

### 현재 코드가 실제로 검사하는 것
- 이 단계 직후 별도 존재 검사는 하지 않습니다.
- 이후 `pipeline_run.py` 내부 분기에서 `generated_taint_config.exists()` 를 확인합니다.
...
- `selected_taint_config` 는 파일명이 아니라 `pipeline_run.py` 내부에서 결정되는 값입니다.
