...
`tools/run_pipeline.py` / `tools/stage/pipeline.py` 기준으로 기록합니다.
...
6. taint config 선택 (`tools/stage/pipeline.py` 내부 분기)
...
`tools/run_pipeline.py full` / `tools/stage/pipeline.py` 는 아래 입력을 받습니다.
...
- 그 외 inventory / CSV / JSON 산출물

### 다음 단계로 넘기는 핵심 출력
- `02a_taint/pulse-taint-config.json` (생성되면 사용)

### 현재 코드가 실제로 하는 일
- comment/code inventory를 만들고 taint config를 생성합니다.
- 이 단계 뒤에 파이프라인 오케스트레이션이 generated taint config 존재 여부를 보고
  이후 Infer에 쓸 config를 결정합니다.

### 현재 코드가 실제로 검사하는 것
- 이 단계 직후 별도 존재 검사는 하지 않습니다.
- 이후 `tools/stage/pipeline.py` 내부 분기에서 `generated_taint_config.exists()` 를 확인합니다.

### downstream이 실제로 쓰는 출력
현재 파이프라인에서 직접 쓰는 것은 사실상 아래 하나입니다.

- `02a_taint/pulse-taint-config.json`

다른 산출물은 현재 기준으로는 주로 보고/분석/디버깅용입니다.
...
- key별 candidate map은 한 번만 생성합니다.
  - 이미 같은 key가 있으면 `duplicate_key_skipped` 로 집계하고 skip 합니다.
- `source_sink_candidate_map.json` 의 value는 call list입니다.
  - 각 call은 `name`, `argc`
  - 필요하면 `original_name` 이 추가될 수 있습니다.
- macro 해석 후 unique function name 집합으로 pulse taint config를 생성합니다.
- pulse taint config는 현재 코드상 항상 아래 구조를 가집니다.
  - `pulse-taint-sources`
  - `pulse-taint-sinks`

### 테스트 작성용 계약 체크리스트
Stage 02a 테스트도 계약 체크와 golden diff를 나누는 것이 좋습니다.
...
- `pulse-taint-config.json` 은 아래 top-level key를 가진다.
  - `pulse-taint-sources`
  - `pulse-taint-sinks`
- `pulse-taint-sources` 의 각 원소는
  - `procedure`
  - `taint_target`
  를 가진다.
- `pulse-taint-sinks` 의 각 원소는
  - `procedure`
  - `taint_target`
  를 가진다.
...
- `pulse-taint-sinks[*].taint_target` 은 현재 구현 기준
  - `AllArguments`
  여야 한다.
- `source_sink_candidate_map.json` 은 JSON object이다.
- `source_sink_candidate_map.json` 의 각 key는 문자열이다.
- `source_sink_candidate_map.json` 의 각 value는 list이다.
- 각 call entry는
  - `name` 비어 있지 않음
  - `argc` 정수, `>= 0`
  - `original_name` 은 optional
- `summary.json` 의 `candidate_map_keys` 는 candidate map key 수와 일치해야 한다.
- `summary.json` 의 `unique_function_names` 는 pulse taint config의 procedure 수와 모순되지 않아야 한다.

#### B. golden regression 테스트
- `code_unique.txt`, `code_frequency.csv`, `function_name_frequency.csv`,
  macro dump, macro resolution CSV 의 정확한 내용
- `source_sink_candidate_map.json` 의 key / value 구조와 내용이 fixture와 정확히 일치한다.
- `pulse-taint-config.json` 의 key / value 구조와 내용이 fixture와 정확히 일치한다.
- 텍스트/CSV/JSON 산출물의 정렬 순서가 fixture와 정확히 일치한다.
- `summary.json` 의 key 집합이 fixture와 정확히 일치한다.
- `summary.json` 의 각 key 값이 fixture와 정확히 일치한다.
  - 현재 구현 기준 주요 summary key:
    - `total_code_entries`
    - `unique_code_entries`
    - `max_frequency`
    - `candidate_map_keys`
    ...
    - `total_function_name_occurrences`
    - `duplicate_key_skipped`
    - `flaw_records_processed`
    - `global_macro_definition_rows`
    - `global_macro_unique_names`
    - `macro_names_detected`
    - `macro_resolved_count`
    - `macro_ambiguous_count`
    - `macro_unresolved_count`
    - `rand_alias_applied_count`

### 테스트에서 계약으로 보기 어려운 것
- macro dump의 정확한 body 순서
- summary JSON 에 생성 시각 같은 비결정적 필드가 추가되는 경우 그 필드 자체
...
### 입력
- `--input-xml 01_manifest/manifest_with_comments.xml`
- `--function-categories-jsonl 02b_flow/function_names_categorized.jsonl`
- `--output-xml 02b_flow/manifest_with_testcase_flows.xml`
- `--summary-json 02b_flow/testcase_flow_summary.json`

### 출력 경로
- `02b_flow/manifest_with_testcase_flows.xml`
- `02b_flow/testcase_flow_summary.json`
...
- flow 하위에 들어갈 수 있는 태그:
  - `comment_flaw`
  - `comment_fix`
  - `flaw`
- flow 안으로 복사된 각 태그에는 `file` 속성이 추가됩니다.
- `flaw` 가 flow 안에 들어간 경우에는 `inferred_function` 속성이 추가될 수 있습니다.
  - 현재 구현상 unresolved flaw는 아예 flow에 들어가지 않으므로,
    flow 안에 남은 `flaw` 는 보통 `inferred_function` 을 가진다고 보는 것이 맞습니다.
...
- flow 하위 태그의
  - `file`
  - `line`
  - 태그 이름(`comment_flaw`, `comment_fix`, `flaw`)

### 현재 코드상 생성 규칙
- testcase에 기존 `<flow>` 태그가 있으면 먼저 제거합니다.
- `comment_flaw`, `comment_fix` 는 `function` 으로 flow를 결정합니다.
- `flaw` 는 같은 file 안의 comment tag line 분포를 이용해 가장 가까운 함수로 `inferred_function` 을 추정합니다.
...
- 각 `<flow>` 하위 태그는 아래 집합 안에 있어야 한다.
  ...
  - `comment_fix`
  - `flaw`
- flow 하위의 모든 태그는 `file` 속성을 가진다.
- flow 하위의 `comment_flaw`, `comment_fix` 는
  - `function` 속성을 유지해야 한다.
  - `line` 은 정수로 파싱 가능해야 한다.
- flow 하위의 `flaw` 는
  - `line` 이 정수로 파싱 가능해야 한다.
  - 가능하면 `inferred_function` 존재를 계약으로 본다.
...
- 각 `flow@type` 별로 포함된 item들의 핵심 정보 집합이 fixture와 일치한다.
  - 예: 태그 이름, `file`, `line`, `function`, `inferred_function`
- summary 의 정확한 수치
...
`tools/stage/pipeline.py` 안에서 **Step 02a 결과를 보고 Step 03에 넘길 taint config 경로를 고르는 분기**입니다.
...
### 실행 위치
- `tools/stage/pipeline.py` 내부 함수 호출
...
- `02b_function_inventory_extract`
  - `manifest_with_comments.xml` → `function_names_unique.csv`
- `02b_function_inventory_categorize`
  - `function_names_unique.csv` + `manifest_with_comments.xml` → `function_names_categorized.jsonl`
- `02b_testcase_flow_partition`
  - `manifest_with_comments.xml` + `function_names_categorized.jsonl` → `manifest_with_testcase_flows.xml`
- taint config 선택
...
- `selected_taint_config` 는 파일명이 아니라 `tools/stage/pipeline.py` 내부에서 결정되는 값입니다.
